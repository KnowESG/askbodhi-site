...
function StepHeader({ num, title, hint }: { num?: string; title?: string; hint?: string }) {
  return (
    <div style={{ marginBottom: 8 }}>
      {num && <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-teal, #0F766E)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Question {num}</div>}
...
export function FieldsStep({ step, answers, canAdvance, setAnswer, goNext, goBack }: FormProps) {
  ...
  return (
    <motion.div key={`fields-${step.num}`} {...fadeUp} style={{ maxWidth: 600, margin: '0 auto' }}>
      <StepHeader num={step.num} title={step.title || ''} hint={step.hint} />
          ...
          return (
            <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
              {fieldGroup.row.map((f) => (
                <div key={f.id}>
                  <label style={{ display: 'block', fontWeight: 500, marginBottom: 8, fontSize: 14, color: 'var(--color-charcoal, #1C1917)' }}>{f.label}</label>
                  <input type={f.type || 'text'} id={f.id} placeholder={f.placeholder} value={answers[f.id] || ''} onChange={(e) => setAnswer(f.id, e.target.value)} required={f.required} style={inputStyle} onFocus={focusHandler} onBlur={blurHandler} />
                </div>
              ))}
            </div>
          );
          ...
          if (isAutocomplete) {
            return (
              <div key={f.id} style={{ marginBottom: 24 }}>
                <label style={{ display: 'block', fontWeight: 500, marginBottom: 8, fontSize: 14, color: 'var(--color-charcoal, #1C1917)' }}>{f.label}</label>
                <Autocomplete items={suggestions} value={answers[f.id] || ''} onChange={(val) => setAnswer(f.id, val)} placeholder={f.placeholder} label="" />
              </div>
            );
          }
          return (
  ...
  return (
    <motion.div key={`text-${step.num}`} {...fadeUp} style={{ maxWidth: 600, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <textarea placeholder={step.field.placeholder} value={answers[step.field.id] || ''} onChange={(e) => setAnswer(step.field.id, e.target.value)}
